...
def test_missing_key(word):
    """
    Fix the missing-key spell errors of the word
    :param word: Any word
    ...
    if word_strip in LEGAL_WORD_FILE:
        ...
        for idx in range(len(word_strip)):
            for item in LOW_ALPHABET:
                fixed_word = word_strip[:idx] + item + word_strip[idx:]
